...
class Field:
	...
	def __str__ ( self ):
		...
		return "%(name)s type: %(type)s [%(required)s] - %(descr)s - [priv: %(private)s]" % dct

	def _get_type ( self, typ ):
		if not typ: return [ FieldType.NONE, None ]
...
class Endpoint:
	...
	def get_return_type ( self ) -> Type :
		return self.parser.structure_get ( self.return_type )
	"""
...
class Module:
	...
	def json_enums ( self, enums ):
		...
		for typ in enums.values ():
			en = D2REnum ( self.parser, None, None )
			en.from_json ( typ )
