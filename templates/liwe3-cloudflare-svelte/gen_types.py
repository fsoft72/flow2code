...
def generate_file_types(self, mod: Module, output: str):
	...
	import { zodBoolean } from '@frontend/liwe3';
    ...
    for type_obj in mod.types.values():
        ...
        for field in type_obj.fields:
            ...
            if not field.name or field.name.strip() == "":
                sys.stderr.write(
                    f"WARNING: Skipping field without name in type '{type_obj.name}'\n"
                )
                continue

            # Get size and min_length, ensuring they're integers
            size = (
                field.size if hasattr(field, "size") and field.size is not None else 0
            )
            min_length = (
                field.min_length
        ...
        zod_schema = zod_schema.replace(
            f"const {original_schema_name}", f"const {new_schema_name}"
        )
