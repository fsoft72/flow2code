...
""",
    "ENDPOINT": """
\tapp.%(__method_lower)s ( '/api%(url)s', %(__perms)sasync ( req: ILRequest, res: ILResponse ) => {
\t\t%(__typed_dict)sconst response = await %(__endpoint_name)s ( req, %(__full_params)s);
\t\tsendResponse( res, response );
\t} );
""",
    "TYPED_DICT": "\t%(name)s: z.%(type)s%(_req_param)s,",
...
%(__snippet)s
""",
    "FUNCTION_END": """	/*=== f2c_end %(function_name)s ===*/%(__post_snip)s
};
""",
